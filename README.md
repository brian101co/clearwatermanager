...
The availability checker allows managers to search by check-in and check-out datetime to find open sites, preventing double bookings.  Available sites display full amenity details including hookup types, lot size and nightly rate.
...
## Payments
The payments system allows managers to track guest payments against reservations. Payments are tracked by status — unpaid, partial, or paid. The payments list provides a summary of outstanding balances with filter tabs for quick access to unpaid and partial payments.
